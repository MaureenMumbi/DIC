		...
		<script type="text/javascript">
			$(document).ready( function () {
				$('#example').dataTable().makeEditable({
                                                                        ...
                                                                        sUpdateURL: "UpdateMarital",
                                                                        sAddURL: "AddMarital",
                                                                        sDeleteURL: "DeleteMarital",
                    							"aoColumns": [  null,                 									
...
<div class="example">
    ...
    <%@include file="../menu/adminmenu.html" %>
		...
		<div id="container">
			...
			<div id="demo">
  <button id="btnAddNewRow" value="Ok">Add new marital status...</button> 
  <button id="btnDeleteRow" value="cancel">Delete selected marital status</button>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
   ...
   <tbody>
        ...
        <c:forEach  var="today" items="${list3}"  >
          ...
          <c:set var="MID"  value="${today.MSTATUS}"></c:set>
           
         <input type="hidden" id="Mid" name="Mid" value="<%= pageContext.getAttribute("ID")%>" />
        <input type="hidden" id="Mid1" name="Mid1" value="<%= pageContext.getAttribute("MID")%>" />
         
            <tr id="<%=pageContext.getAttribute("ID")%>">
           
           <td>  ${today.ID} </td>
           <td class="sorting_1">${today.MSTATUS} </td>
     ...
     <form id="formAddNewRow" action="#" title="Add new Marital Status">
			    <input type="hidden" id="id" name="id" rel="0" />
			    <label for="name">Code</label><input type="text" name="maritalStatus" id="maritalStatus" class="required" rel="1" />
