 ...
 * @author AphiaPlus
 */
public class rrreport extends HttpServlet {
...
int totalperrow=0;
   HSSFCell  cells5, cells6, cells7, cells8, cells9, cells10;
        if(dics.get(u)!=null && !dics.get(u).equals("")){            
            ...
            HSSFRow rwc= shet1.getRow(shet1.getFirstRowNum());
    
            a=3;
            for(int g=0;g<questions.length;g++){                             
         rwc = shet1.createRow(a+g);
    }
...
"	and  Enrollment.DICName='"+dics.get(u).toString()+"'  AND RiskReductionDetails.QID='B1' AND RiskReductionMain.qtr='"+quarters2[z]+"' AND RiskReductionMain.year='"+reportyear+"'"+
