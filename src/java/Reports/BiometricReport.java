 ...
 */
public class BiometricReport extends HttpServlet {
...
int year_pos;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (report_type.equals("Biometric")){
...
//          
   ...
   cell000.setCellValue("DIC BIOMETRIC ENROLLMENTS REPORTS");
   ...
   conn.rs=conn.state.executeQuery(DIC_Name_fetcher);
     ...
     ///End of getting the Pefar years
        // while(anza <= kwisha){
          ...
          String DIC_count_enrol="select count(DISTINCT UniqueID),DICName,QUARTER(STR_TO_DATE(DOE,'%e/%c/%Y')),Pefar_year,DOE from enrollment where DICName='"+ dic_name+"' AND (STR_TO_DATE(DOE,'%e/%c/%Y')) BETWEEN (STR_TO_DATE('"+start_date+"','%e/%c/%Y')) AND (STR_TO_DATE('"+end_date+"','%e/%c/%Y')) and  fingerprint!='' GROUP BY QUARTER(STR_TO_DATE(DOE,'%e/%c/%Y')),DICName,Pefar_year ";
...
response.setHeader("Content-Disposition", "attachment; filename=DIC_Biometric_Enrollment_Quarter_Report.xls");
...
response.setContentLength(outArray.length);
response.setHeader("Expires:", "0"); // eliminates browser caching
response.setHeader("Content-Disposition", "attachment; filename=DIC_Biometric_Enrollment_Monthly_Report.xls");
OutputStream outStream = response.getOutputStream();
outStream.write(outArray);
outStream.flush();
            }
